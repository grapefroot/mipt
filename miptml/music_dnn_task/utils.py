...
def train_net(net, train_fun, test_fun, X_train, y_train, X_valid, y_valid, num_epochs=10, batch_size=5):
	...
	if(epoch > 0 and epoch % 25 == 0):
		np.save('epoch_time_valacc {}:{}:{}'.format(epoch, datetime.now(), val_acc/val_batches*100), layers.get_all_params(net)) 
    np.save('final weights for {}. train_acc = {}, val_acc = {}'.format(num_epochs, train_acc/train_batches*100, val_acc/val_batches*100), layers.get_all_params(net)) 	
    return net
